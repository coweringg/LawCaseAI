...
export default function AuthLayout({ children, sideContent, noScroll = false }: AuthLayoutProps) {
    return (
        <div className={`bg-background-dark text-slate-100 antialiased h-screen w-full flex overflow-hidden font-display transition-colors duration-200 ${noScroll ? 'fixed inset-0' : ''}`}>
            <div className="hidden lg:flex w-[45%] bg-background-dark relative flex-col justify-between overflow-hidden border-r border-white/5 shadow-2xl">
                <div className="absolute inset-0 z-0 overflow-hidden">
                    <div className="absolute inset-0 bg-background-dark"></div>
                    <div className="absolute inset-0 micro-grid opacity-30"></div>
                ...
                <div className="relative z-10 px-12 py-12 h-full flex flex-col justify-between">
                    <Link href="/" className="flex items-center gap-3 group">
                        <MonolithLogo size={40} glowIntensity="lg" className="group-hover:scale-110 transition-transform duration-300" />
                    ...
                    >
                        <div className="relative z-10">
                            <div className="flex items-center gap-1 mb-6 text-primary drop-shadow-[0_0_10px_rgba(0,230,118,0.4)]">
                                {[1, 2, 3, 4, 5].map(i => (
                            ...
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full bg-primary flex items-center justify-center text-xs font-black text-background-dark shadow-lg">
                                    JD
                                </div>
                                <div>
            ...
            <div className={`w-full lg:w-[55%] h-full bg-background-dark flex flex-col items-center justify-center p-4 sm:p-8 relative ${noScroll ? 'overflow-hidden' : 'overflow-y-auto'} scrollbar-hide`}>
                <div className="lg:hidden absolute top-6 left-6 flex items-center gap-2">
                    <Link href="/" className="flex items-center gap-2.5">
                        <MonolithLogo size={32} glowIntensity="md" />
                        <span className="text-white font-bold text-lg">LawCaseAI</span>
                    </Link>
                </div>
