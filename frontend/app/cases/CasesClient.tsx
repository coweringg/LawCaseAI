...
const getAreaColor = (area: string) => {
    ...
    if (a.includes('civil') || a.includes('litigation')) return 'bg-primary/10 text-primary dark:bg-primary/10 dark:text-primary';
            ...
            >
                <motion.div variants={itemVariants} className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-4">
                    <div>
                        ...
                        <div className="flex items-center gap-3">
                            <span className="w-2 h-2 rounded-full bg-primary animate-pulse shadow-[0_0_10px_rgba(0,230,118,0.8)]"></span>
                ...
                {isLoading ? (
                    <div className="flex flex-col justify-center items-center py-32 gap-6">
                        <div className="w-12 h-12 border-4 border-primary/20 border-t-primary rounded-full animate-spin shadow-[0_0_20px_rgba(0,230,118,0.2)]"></div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                        <Link href="/cases/new" className="group">
                            ...
                            >
                                <div className="absolute inset-0 micro-grid opacity-30 z-0 pointer-events-none transition-transform duration-200 group-hover:scale-150"></div>
                                <div className="relative z-10 flex flex-col items-center">
                                    <div className="w-20 h-20 rounded-3xl bg-white/5 flex items-center justify-center mb-8 group-hover:bg-primary group-hover:text-background-dark group-hover:rotate-12 transition-all duration-200 shadow-2xl border border-white/5 group-hover:shadow-[0_0_30px_rgba(0,230,118,0.4)]">
                                        <Plus className="w-10 h-10" />
                                    </div>
                                    <span className="text-xl font-black text-white font-display tracking-tightest">Initialize Case</span>
                                    <span className="text-[9px] font-black uppercase tracking-[0.3em] mt-3 text-primary transition-colors">Neural Core Expansion</span>
                                </div>
                            </motion.div>
                        </Link>
                                    ...
                                    <motion.div
                                        ...
                                        className={`premium-glass border rounded-[2.5rem] p-8 transition-all duration-300 h-full flex flex-col relative overflow-hidden shadow-2xl group
                                            ${isClosed 
                                                ...
                                                : isDiscovery
                                                ? 'border-white/10 hover:border-primary/40 opacity-90 hover:opacity-100 hover:shadow-[0_0_30px_rgba(0,230,118,0.15)]'
                                                : 'border-white/10 hover:border-primary/40 hover:shadow-[0_0_30px_rgba(0,230,118,0.15)]'
                                            }
                                        `}
                                    >
                                        <div className={`absolute inset-0 bg-gradient-to-br to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none
                                            ${isClosed ? 'from-white/[0.03]' : isPending ? 'from-amber-500/[0.04]' : 'from-primary/[0.04]'}
                                        `}></div>
                                        <div className="absolute inset-0 micro-grid opacity-30 pointer-events-none transition-opacity duration-300"></div>
                                        ...
                                        <div className="relative z-10 flex-1 flex flex-col">
                                            <h3 className={`text-2xl font-black mb-3 transition-colors leading-tight font-display tracking-tightest text-white
                                                ...
                                                ${isDiscovery && 'group-hover:text-primary'}
                                            `}>{c.name}</h3>
                                                ...
                                                {c.complexity && (
                                                    <>
                                                        ...
                                                        <span className={`text-[10px] font-black uppercase tracking-widest ${
                                                            c.complexity === '1' ? 'text-primary/70' :
                                                            c.complexity === '2' ? 'text-primary/70' :
                                                            'text-rose-500/70'
                                                        }`}>
                                                            Lvl {c.complexity}
                                                        </span>
                                            ...
                                            <div className="w-full bg-white/5 h-2 rounded-full overflow-hidden mb-6 shadow-inner">
                                                <motion.div
                                                    ...
                                                    className={`h-full rounded-full transition-all duration-150 ${progress === 100 
                                                        ? 'bg-primary shadow-[0_0_15px_rgba(0,230,118,0.5)]' 
                                                        : 'bg-primary shadow-[0_0_15px_rgba(0,230,118,0.4)]'}`}
                                                ></motion.div>
                                            </div>
